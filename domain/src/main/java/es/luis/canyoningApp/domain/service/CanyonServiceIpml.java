...
public class CanyonServiceIpml implements CanyonService {
    ...
    public Canyon getCanyonById(Long canyonId) {
        return canyonRepository.getCanyonById(canyonId);
    }
    ...
    @Override
    public void downloadCanyon(Long canyonId) {
        ...
        try (PDDocument document = new PDDocument()) {
            ...
            try (PDPageContentStream contentStream = new PDPageContentStream(document, page)) {
                ...
                if (!ObjectUtils.isEmpty(canyon.getCanyonProhibition())) {
                    ...
                    canyon.getCanyonProhibition().forEach(canyonProhibition -> {
                        try {
                ...
                if (!ObjectUtils.isEmpty(canyon.getCanyonDifficulty())) {
                    ...
                    canyon.getCanyonDifficulty().forEach(canyonDifficulty -> {
                        try {
                            addText(contentStream, "Descenso " + canyonDifficulty.getDescentNumber() + ":" + canyonDifficulty.getDifficultyDesc(), "text");
                        } catch (IOException e) {
                            throw new RuntimeException(e);
                        }
                    });
                ...
                if (!ObjectUtils.isEmpty(canyon.getCanyonLink())) {
                    ...
                    canyon.getCanyonLink().forEach(canyonLink -> {
                        try {
                            addText(contentStream, canyonLink.getTitle() + ":" + canyonLink.getLink(), "text");
                        } catch (IOException e) {
                            throw new RuntimeException(e);
                        }
                    });
                ...
                if (!ObjectUtils.isEmpty(canyon.getCanyonDescent())) {
                    ...
                    canyon.getCanyonDescent().forEach(canyonDescent -> {
                        try {

                            addText(contentStream, "Descenso " + canyonDescent.getDescentNumber() + ": Longitud = " + canyonDescent.getLength() + ", Desnivel = " + canyonDescent.getSlope() + ", Numero de rapeles: " + canyonDescent.getRapelNum() + ", Longitud maxima en un rapel = " + canyonDescent.getMaxLength() + ", Estado del equipamiento = " + canyonDescent.getEquipment(), "text");
                        } catch (IOException e) {
                            throw new RuntimeException(e);
                        }
                    });
                ...
                if (!ObjectUtils.isEmpty(canyon.getCanyonSchedule())) {
                    ...
                    canyon.getCanyonSchedule().forEach(canyonSchedule -> {
                        try {

                            addText(contentStream, "Coches: " + canyonSchedule.getCar() + ", Numero de descenso: " + canyonSchedule.getDescentNumber() + ", Aproximacion = " + canyonSchedule.getApproach() + ", Descenso = " + canyonSchedule.getDescent() + ", Retorno = " + canyonSchedule.get_return(), "text");
                        } catch (IOException e) {
                            throw new RuntimeException(e);
                        }
                    });
            ...
            try (PDPageContentStream contentStream = new PDPageContentStream(document, page2)) {
                ...
                if (!ObjectUtils.isEmpty(canyon.getCanyonRappeling())) {
                    ...
                    canyon.getCanyonRappeling().stream()
                            .sorted((c1, c2) -> compareSteps(c1.getStep(), c2.getStep())).forEach(canyonRappeling -> {

                                try {
                                    if (!descent[0].equals(canyonRappeling.getDescentNumber())) {
                                        descent[0] = canyonRappeling.getDescentNumber();
                                        addText(contentStream, "Descenso: " + canyonRappeling.getDescentNumber(), "secondTitle");
                                    }
                                    addText(contentStream, canyonRappeling.getStep() + "   |   " + canyonRappeling.getStepType() + "   |   " + canyonRappeling.getLength() + "   |   " + canyonRappeling.getLocation() + "   |   " + canyonRappeling.getDescription(), "text");
            ...
            document.save("Torrent_Rec_de_Cabanelles.pdf");
    ...
    private static void addText(PDPageContentStream contentStream, String text, String textType) throws IOException {
    ...
    private static List<String> splitTextIntoLines(String text, PDType1Font font, float sizeFont) throws IOException {
