...
public class CanyonServiceIpml extends BaseService implements CanyonService {
  ...
  public Canyon createCanyon(Canyon canyon) {
    ...
    if (!ObjectUtils.isEmpty(canyon.getCroquis())) {
      canyon.setCroquis(canyon.getCroquis().replace(" ", "_"));
    }
    if (getAuthenticatedUserEmail().equals("delgadofernandez.luis@gmail.com")) {
      Canyon canyonSaved = canyonRepository.createCanyon(canyon);
    }
    sendEmail.sendCreateCanyon(canyon, getAuthenticatedUsername());
  ...
  public Canyon getCanyonById(Long canyonId) {
    ...
    webClient
        .post()
        .uri("http://127.0.0.1:8000/search/canyons")
  ...
  public Canyon updateCanyon(Long canyonId, Canyon canyon) {
    ...
    // todo solo se podra cambiar el nombre si el nombre anterior esta completo en el nuevo nombre,
    // es decir barranco de agua -> barranco de agua limpia
    ...
    sendEmail.sendUpdateCanyon(canyonSaved, canyon, getAuthenticatedUsername());
    ...
    try (PDDocument document = new PDDocument()) {
      ...
      try (PDPageContentStream contentStream = new PDPageContentStream(document, page)) {
        ...
        if (!ObjectUtils.isEmpty(canyon.getSeason())) {
          addText(contentStream, "Temporada ideal:", "secondTitle");
      ...
      String pathname =
          canyoningAppConfiguration.getCroquis()
              + canyoningAppConfiguration.getSlash()
              + canyon.getCroquis();
      log.info(pathname);
      ...
      for (CanyonControlLevel controlLevel : canyonControlLevel) {
        ...
        pathname =
            canyoningAppConfiguration.getControlPoint()
                + canyoningAppConfiguration.getSlash()
                + controlLevel.getName();
