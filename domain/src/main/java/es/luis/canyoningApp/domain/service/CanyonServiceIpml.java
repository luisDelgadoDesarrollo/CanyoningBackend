...
public class CanyonServiceIpml extends BaseService implements CanyonService {
    ...
    public Canyon createCanyon(Canyon canyon) {
        ...
        canyon.setCroquis(canyon.getCroquis().replace(" ", "_"));
    ...
    public Resource downloadCanyon(Long canyonId, Boolean email) {
        ...
        try (PDDocument document = new PDDocument()) {
            ...
            Ad ad = adRepository.getAdByDate(LocalDate.now());

            if (ad != null) {
                PDPage pageAnuncio = new PDPage();

                String anuncio =
                        canyoningAppConfiguration.getBasePath() + canyoningAppConfiguration.getAd();
                // todo comentar la doble barra antes de subir nada pues en linux petaria
                String anuncioCompleto;
                if (anuncio.contains("\\")) {
                    anuncioCompleto = anuncio + "\\\\" + ad.getName();
                } else {
                    anuncioCompleto = anuncio + ad.getName();
                }
                log.info(anuncioCompleto);
                File imageFileAnuncio = new File(anuncioCompleto);
                if (imageFileAnuncio.exists()) {
                    document.addPage(pageAnuncio);
                    try (PDPageContentStream contentStream = new PDPageContentStream(document, pageAnuncio)) {
                        contentStream.beginText();
                        contentStream.newLineAtOffset(50, 750);
                        addText(contentStream, "Publicidad", "title");
                        contentStream.endText();
                        PDImageXObject image = PDImageXObject.createFromFile(anuncioCompleto, document);
                        // Obtener el tamaño de la página
                        float pageWidth = pageAnuncio.getMediaBox().getWidth();
                        float pageHeight = pageAnuncio.getMediaBox().getHeight();
                        ...
                        // Definir la posición del enlace (toda la página)
                        PDRectangle linkRect = new PDRectangle(0, 0, pageWidth, pageHeight);
                        link.setRectangle(linkRect);

                        // Definir la acción de redirección al enlace
                        PDActionURI action = new PDActionURI();
                        action.setURI(ad.getLink());
                        link.setAction(action);

                        // Agregar el enlace a la página
                        pageAnuncio.getAnnotations().add(link);
                    }
                }
            }
            ...
            downloadRepository.saveDownload(canyonId, getAuthenticatedUserId(), OffsetDateTime.now());
